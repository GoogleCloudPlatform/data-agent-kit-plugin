...
-   Prevent group data leakage: for item-level datasets (e.g., inventory
    items), aggregate records by entity (e.g., `product_id`) prior to train/test
    split to prevent multiple instances of the same entity from leaking across
    splits.
